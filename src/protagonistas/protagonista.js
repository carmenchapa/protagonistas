  ...
  render() {
    ...
    return (
      <ResponsiveLayout
        breakPoint={800}
        renderDesktop={() => (
          <DesktopScreen protagonista={protagonista} name={name} />
        )}
        renderMobile={() => (
          <MobileScreen protagonista={protagonista} name={name} />
        )}
      />
    )
  }
...
const MobileScreen = props => {
  ...
  return (
    <div className="AppContainer Dark">
      <div
        className="Centred ProtaImgContainer"
        style={GradientBackgroundImage(protagonista.img)}
      >
        <p className="Name">{name.toUpperCase()}</p>
        <p className="Text">{protagonista.shortText}</p>
        <div className="Column">
          {protagonista.instagram && (
            <SocialIcon link={protagonista.instagram} icon="instagram" />
          )}
          {protagonista.facebook && (
            <SocialIcon link={protagonista.facebook} icon="facebook" />
          )}
        </div>
      </div>
      <div
        className="Column Dark"
        style={{padding: 15, justifyContent: "centre"}}
      >
        {newText(protagonista.text)}
